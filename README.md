...
Set `DATABASE_URL` to a production Postgres connection string and run Prisma migrations before starting the app. NDL stores proof links by default. Keep `UPLOAD_MODE=disabled` for serverless production hosting unless you add a persistent storage provider. Full GitHub, Vercel, Neon, SMTP, and Namecheap notes are in [docs/deployment.md](docs/deployment.md).
